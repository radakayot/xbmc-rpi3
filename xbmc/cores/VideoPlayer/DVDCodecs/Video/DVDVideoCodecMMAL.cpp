...
bool CDVDVideoCodecMMAL::Open(CDVDStreamInfo& hints, CDVDCodecOptions& options)
{
  ...
  mmal_port_parameter_set_uint32(m_input, MMAL_PARAMETER_VIDEO_MAX_NUM_CALLBACKS, -MMAL_CODEC_NUM_BUFFERS);
...
CDVDVideoCodec::VCReturn CDVDVideoCodecMMAL::GetPicture(VideoPicture* pVideoPicture)
{
  ...
  if (state == MCS_INITIALIZED || state == MCS_UNINITIALIZED || state == MCS_ERROR)
  {
    result = VC_ERROR;
